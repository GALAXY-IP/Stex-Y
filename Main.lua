...
local WindUI
do
    -- Coba load dari local (untuk Roblox Studio)
    ...
    if ok then
        ...
        -- Jika gagal (untuk executor online), ambil dari GitHub raw URL
        -- ./src/Init -> https://raw.githubusercontent.com/Footagesus/WindUI/main/src/Init.lua
        local githubUrl = "https://raw.githubusercontent.com/Footagesus/WindUI/main/src/Init.lua"
        WindUI = loadstring(game:HttpGet(githubUrl))()
    end
end
...
ProfileTab:Button({
    ...
    Callback = function()
        ...
        WindUI:Notify({Title = "✅ Info Diperbarui", Content = "Profil berhasil diperbarui"})
    end
})
    ...
    Callback = function()
        if States.selectedPlayer and States.selectedPlayer ~= "Tidak ada player lain" then
            ...
            if target and target.Character and target.Character:FindFirstChild("HumanoidRootPart") then
                ...
                WindUI:Notify({Title = "✅ Teleport Berhasil", Content = "Ke " .. States.selectedPlayer})
            else
                WindUI:Notify({Title = "❌ Teleport Gagal", Content = "Player tidak ditemukan"})
            end
        else
            WindUI:Notify({Title = "⚠️ Pilih Player", Content = "Silakan pilih player terlebih dahulu"})
        end
    end
})
        ...
        if States.selectedRegion then
            ...
            if regionFolder then
                ...
                if targetRegion and targetRegion:FindFirstChild("Spawn") then
                    ...
                    WindUI:Notify({Title = "✅ Teleport Berhasil", Content = "Ke " .. States.selectedRegion})
                else
                    WindUI:Notify({Title = "⚠️ Region Tidak Ditemukan", Content = "Mencari alternatif.."})
                end
            else
                WindUI:Notify({Title = "❌ Region Gagal", Content = "Folder region tidak ditemukan"})
            end
        end
    end
})
...
TeleportTab:Button({
    ...
    Callback = function()
        ...
        WindUI:Notify({Title = "✅ Refresh Berhasil", Content = "Daftar player diperbarui"})
    end
})
...
InfoTab:Button({Title = "Copy Telegram", Icon = "copy", Callback = function() setclipboard("@tarzbot") WindUI:Notify({Title = "✅ Disalin", Content = "Telegram: @tarzbot"}) end})
InfoTab:Button({Title = "Copy WhatsApp", Icon = "copy", Callback = function() setclipboard("0812345678") WindUI:Notify({Title = "✅ Disalin", Content = "WhatsApp: 0812345678"}) end})
InfoTab:Button({Title = "Copy TikTok", Icon = "copy", Callback = function() setclipboard("@_tarzbot") WindUI:Notify({Title = "✅ Disalin", Content = "TikTok: @_tarzbot"}) end})
